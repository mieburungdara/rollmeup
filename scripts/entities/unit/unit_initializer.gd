...
static func initialize_unit_data(u: CharacterBody2D) -> void:
	...
	u.unit_name = (
		NameData.NAMES[u.u_race].pick_random() if NameData.NAMES.has(u.u_race) else r_data.name
	)
...
static func generate_random_traits(u: CharacterBody2D) -> void:
	...
	if u.u_race == RaceData.RaceType.ELF:
		u.v_head = 1
	elif u.u_race == RaceData.RaceType.ORC:
		u.v_head = 2
	elif u.u_race == RaceData.RaceType.DWARF:
		u.v_head = 3
		u.v_hair = 3

	var trait_pool = []
	for t in TraitData.TraitType.values():
		if t != TraitData.TraitType.NONE:
			trait_pool.append(t)
	trait_pool.shuffle()

	var target_count = randi_range(3, 5)
	for t_id in trait_pool:
		if u.u_traits.size() >= target_count:
			break
		var has_conflict = false
		if TraitData.CONFLICTS.has(t_id):
			for cid in TraitData.CONFLICTS[t_id]:
				if cid in u.u_traits:
					has_conflict = true
					break
		if not has_conflict:
			u.u_traits.append(t_id)
...
static func apply_all_modifiers(u: CharacterBody2D) -> void:
	...
	for t_id in u.u_traits:
		if TraitData.TRAITS.has(t_id):
			...
			if t_data.has("hp_mult"):
				u.stats.max_hp *= t_data.hp_mult
			if t_data.has("dmg_mult"):
				u.stats.damage *= t_data.dmg_mult
			if t_data.has("speed_mult"):
				u.stats.speed *= t_data.speed_mult
			if t_data.has("scale"):
				u.base_scale *= t_data.scale
	u.stats.hp = u.stats.max_hp
...
static func setup_visuals(u: CharacterBody2D) -> void:
	var apply_part = func(sprite, tex_path, variant):
		...
		sprite.region_rect = Rect2(variant * 32, 0, 32, 32)
	...
	var r_data = RaceData.RACES[u.u_race]
	var skin = r_data.skin_colors.pick_random()
	var hair = r_data.hair_colors.pick_random()
	u.sprite_head.modulate = skin
	u.sprite_arms.modulate = skin
	u.sprite_legs.modulate = skin
	u.sprite_hair.modulate = hair
	u.sprite_body.material.set_shader_parameter(
		"kingdom_color", u.kingdom_color if u.kingdom_id != 0 else Color.WHITE
	)
